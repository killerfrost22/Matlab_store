...
eps/2 - d
%%
%ans = 5.551115123125783e-17 > 0 so the d satisfies the upperbound 
%d ≤ε_mach/2.
...
% f~(h) you propose, and report your results for h = 10^−1, 10^-2, ..., 10^-18 on a
...
x = 10*ones(1,18); y = 1:1:18; h = x.^ (-y);
semilogx(h,f(h),h,fNew(h),'--')
title 'subtracting rounded equal numbers'; legend({'f(h)','fNew(h)'});

%%
%Since h→0, the lim_h→0 f(h) should be 32. However, for very small h the error starts
%increasing due to the subtraction of nearly equal numbers.
%Modified function computes more accurately for smaller h,the dash line 
%represent the modified function and lim_h→0 fNew(h) = 32.
%% Problem 3 (Optional)
%%
...
%Matlab will "ignore" b and return c=sqrt(a^2)， since a will be much bigger than b
%    
%    
%%
